...
//// SQL-RELATED
    ...
    public function load_validation($context, $course) {
        ...
        if (is_numeric($this->sortitemid)) {
            $params = array_merge(array('gitemid'=>$this->sortitemid), $gradebookrolesparams, $enrolledparams);
        ...
        foreach ($this->users as $userid => $user) {
            ...
            foreach ($this->quizzes as $quizid => $quiz) {
                ...
                if (array_key_exists($userid, $this->quizauths) && array_key_exists($quizid, $this->quizauths[$userid])) {
                    $quizcell->text .= $this->make_decision_output($this->quizauths[$userid][$quizid][10]);
                } else {
                    $quizcell->text .= '-';
                }
